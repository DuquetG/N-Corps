...
subroutine Lyapunov(nb_simulations, Nstep, nbCorps, chosen_mass, dt, length, pasCalcul)
implicit none
    ...
    do i = 1, nb_simulations-1
        ...
        delZero2=0
        do m=0,nbCorps-1
            delZero2=delZero2+(Xstep(nbCorps*2*i+2*m)-Xstep(2*m))**2 &
            + (Xstep(nbCorps*2*i+2*m+1)-Xstep(2*m+1))**2 &
    ...
    do line = 1, length/nb_simulations-1
        ...
        do j = 1, nb_simulations-1
            ...
            del2=0
            do n=0,nbCorps-1
                del2=del2+(Xstep(nbCorps*2*j+2*n)-Xstep(2*n))**2 &
                + (Xstep(nbCorps*2*j+2*n+1)-Xstep(2*n+1))**2 &
